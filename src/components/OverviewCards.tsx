...
const StatCard = ({ title, value, percentage, type }: { title: string; value: string | number; type: string; percentage?: number }) => {
  const isPositive = type === 'percentage' && (percentage ?? 0) >= 0;
...
const OverviewCards = () => {
  ...
  const cardData: { title: string; value: string | number; type: string; percentage?: number }[] = [
