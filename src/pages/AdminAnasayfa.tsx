...
const AdminAnasayfa = () => {
  ...
  const [fundSeriesData, setFundSeriesData] = useState<Record<string, { date: string | null; price: number }[]>>({});
  const [marketData, setMarketData] = useState<{ bist: { date: string; price: unknown }[]; usd: { date: string; price: unknown }[] }>({ bist: [], usd: [] });
  ...
  const chartData = useMemo(() => {
    ...
    const fundMaps: Record<string, Record<string, number>> = {};
    Object.entries(fundSeriesData).forEach(([code, data]) => {
      fundMaps[code] = {};
      data.forEach(d => { if (d.date) { dateSet.add(d.date); fundMaps[code][d.date] = d.price; } });
    });
    const bistMap: Record<string, unknown> = {};
    if (showBist) marketData.bist.forEach(d => { dateSet.add(d.date); bistMap[d.date] = d.price; });
    const usdMap: Record<string, unknown> = {};
